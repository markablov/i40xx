...
const PI = '';

(function main() {
  const dirname = path.dirname(fileURLToPath(import.meta.url));
  const intermediateChecks = JSON.parse(fs.readFileSync(path.resolve(dirname, './pi255_split.dat'), 'utf8'));

  const { roms } = compileCodeForTest('pi255_split.i4040', '', { wrapSourceCode: (sourceCode) => sourceCode });
  ...
  while (!system.isFinished()) {
    if (labelOffsetForChecks.has(system.registers.pc) && system.registers.selectedRomBank === 0) {
      ...
      for (const [idx, expected] of intermediateChecks[`${m}_${negN}`].entries()) {
        ...
        if (result !== expected) {
          console.log(`  failed chunk #${idx}, f = ${f}, m = ${m}, N = ${(0x100 - negN) + 1}`);
